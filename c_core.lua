...
-- *
-- * We highly discourage editing the files for uses on public servers.
-- * Make use of the customization possibilities instead.
---------------------------------------------------
...
function Core.onSettingsReceived(lSettingsTable)
	...
	if(Core.g_SettingsTable["bouncecommands"]["disable"] == false and Core.g_SettingsTable["bouncebind"]["disable"] == false) then
		if(Core.g_SettingsTable["infomessage"]["disable"] == false) then
			local lShowMessage = Core.g_SettingsTable["infomessage"]["value"]
	...
	elseif(Core.g_SettingsTable["bouncecommands"]["disable"] == false or Core.g_SettingsTable["bouncebind"]["disable"] == false) then
		if(Core.g_SettingsTable["infomessage2"]["disable"] == false) then
			local lShowMessage = Core.g_SettingsTable["infomessage2"]["value"]
			
			if(Core.g_SettingsTable["bouncebind"]["disable"] == false) then
				lShowMessage = lShowMessage:gsub("%%1",tostring(Core.g_SettingsTable["bouncebind"]["value"]))
			else
				lShowMessage = lShowMessage:gsub("%%1","/"..tostring(Core.g_SettingsTable["bouncecommands"]["value"]:gsub(","," /")))
			end
